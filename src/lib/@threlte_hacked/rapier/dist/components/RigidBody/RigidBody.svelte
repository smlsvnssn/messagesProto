<script>
	...
	const { world, rapier, addRigidBodyToContext, removeRigidBodyFromContext } =
		useRapier()
	 ...
	 */
	const desc = new rapier.RigidBodyDesc(parseRigidBodyType(type)).setCanSleep(
		canSleep,
	)
	/**
	...
	const initPosition = async () => {
		...
		object.updateMatrix()
		object.updateWorldMatrix(true, false)
		const parentWorldScale = object.parent
			? getWorldScale(object.parent)
			: new Vector3(1, 1, 1)
		const worldPosition =
			getWorldPosition(object).multiply(parentWorldScale)
