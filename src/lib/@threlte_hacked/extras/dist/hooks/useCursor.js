...
import { useThrelte } from '$lib/@threlte_hacked/core'
export const useCursor = (onPointerOver = 'pointer', onPointerOut = 'auto') => {
	...
	let onPointerOverValue =
		typeof onPointerOver === 'string' ? onPointerOver : get(onPointerOver)
	if (typeof onPointerOver !== 'string') {
		const unsubscribeOnPointerOver = onPointerOver.subscribe(
			cursorStyle => {
	...
	let onPointerOutValue =
		typeof onPointerOut === 'string' ? onPointerOut : get(onPointerOut)
