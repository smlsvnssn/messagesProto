<script>
	...
	const transformElStyles = derived(
		[styles.transform.el, styles.common.el],
		([vA, vB]) => {
			return {
				...vA,
				...vB,
	...
	const portalAction = el => {
		if (!renderer) {
			console.warn(
				'HTML: renderer is undefined. Is this component a child to <Canvas>?',
			)
			return
		}
