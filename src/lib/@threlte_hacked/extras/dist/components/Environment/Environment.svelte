<script>
	import { useParent, useThrelte } from '$lib/@threlte_hacked/core'
	...
	import { HDRCubeTextureLoader } from '../../hacked_three_jsm_examples/loaders/HDRCubeTextureLoader'
	import { RGBELoader } from '../../hacked_three_jsm_examples/loaders/RGBELoader'
	...
	const pickLoader = () => {
		const inferredFormat =
			format ||
			(Array.isArray(files) ? files[0] : files).split('.').pop() == 'hdr'
				? 'hdr'
				: 'ldr'
	...
	const loadEnvironment = () => {
		...
		loader.setPath(path || '').load(files, texture => {
			texture.mapping = isCubeMap
				...
				: EquirectangularReflectionMapping
			texture.encoding =
				encoding || isCubeMap ? LinearEncoding : sRGBEncoding
...
{#if groundProjection}
	<GroundProjectedEnv {groundProjection} currentEnvMap={previousEnvMap} />
{/if}
