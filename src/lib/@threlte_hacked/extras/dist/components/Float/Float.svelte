<script>
	...
	import { useFrame, Group } from '$lib/@threlte_hacked/core'
	// Group Properties
	...
	let combinedRotation = {
		x:
			rotation?.x ??
			0 + (Math.cos((t / 4) * speed) / 8) * rotationIntensity,
		y:
			rotation?.y ??
			0 + (Math.sin((t / 4) * speed) / 8) * rotationIntensity,
		z:
			rotation?.z ??
			0 + (Math.sin((t / 4) * speed) / 20) * rotationIntensity,
	}
