...
<script>
	...
	import { Three } from '$lib/@threlte_hacked/core'
	...
	const makeGridMaterial = axes => {
      ...
      void main() {
        ...
        pos.${axes.slice(0, 2)} += (cameraPosition.${axes.slice(
				0,
				2,
			)} * uFollowCamera);
      ...
      void main() {
        ...
        float d = 1.0 - min(distance(cameraPosition.${axes.slice(
			0,
			2,
		)}, worldPosition.${axes.slice(0, 2)}) / uFadeDistance, 1.);
