<script>
	...
	import { Line2 as ThreeLine2 } from '../hacked_three_jsm_examples/lines/Line2'
	import { LineGeometry } from '../hacked_three_jsm_examples/lines/LineGeometry'
	...
	$: {
		...
		if (points.length) {
			...
			geometry.setPositions(
				points
					.map(p => (p instanceof Vector3 ? p.toArray() : p))
					.flat(),
			)
